...
pool = gcp('nocreate');
if ~isempty(pool)
    delete(pool);              % Close current pool (4 workers)
end

parpool('local', 8);           % Now reopen with 8 workers
...
% Return 1xN cell array in header order. Honor LITE gating.
    ...
    % ratios (always compute if requested, even in LITE)
...
% Full-window stats with LITE gating.
        ...
        if region_included
            ...
            if LITE.compute_entropy
                ...
                if isempty(chanIdx)
                    ...
                    seVals = nan(1, numel(chanIdx));
                    for ii = 1:numel(chanIdx)
                        x = double(win(chanIdx(ii), :));
                        if ~all(isfinite(x))
                            seVals(ii) = NaN; 
                            continue
                        end
                        ...
                        try
                            [SampVec, ~, ~] = SampEn(x, 'm', 2, 'tau', 1, 'r', rtol, ...
                                                      'Logx', exp(1), 'Vcp', false);
                            seVals(ii) = SampVec(2);   % m=2, nats
                        catch
                            seVals(ii) = NaN;
                        end
                    end
                    feats.(['Full_' rname '_SampleEntropy']) = mean(seVals, 'omitnan');
                    fprintf('  [%s | %s] SampEn = %.4f\n', ...
    ...
    % % --------- 1/f (aperiodic) slope & intercept ------------------------------
    % %   • Fit 2–30 Hz PSD excluding 8–13 Hz (alpha) to reduce peak bias.
    % %   • Sanity checks: R² > 0.8 & slope between –4 and –0.5.
    % 
    % fmask_1f = (f >= 2 & f <= 30) & ~(f >= 8 & f <= 13);
    % for r = ["FrontalMidline","Parietal"]
    %     mask = ismember(chanlabels, regs.(r));
    %     if any(mask) && any(fmask_1f)
    %         psd_r = squeeze(mean(PSD(mask,fmask_1f,:),1));     % [freq x timewin]
    %         psd_mean = mean(psd_r,2);                          % average over time
    %         logf  = log10(f(fmask_1f));
    %         logps = log10(psd_mean);
    %         p = polyfit(logf, logps, 1);
    %         slope = p(1);
    %         intercept = p(2);
    %         % simple R² sanity check
    %         yhat = polyval(p, logf);
    %         R2 = 1 - sum((logps - yhat).^2) / sum((logps - mean(logps)).^2);
    %        % if R2 < 0.8 || slope > -0.5 || slope < -4
    %         %    slope = NaN; intercept = NaN; % fail sanity
    %         %end
    %         feats.(sprintf('Full_%s_aperiodic_slope',r)) = slope;
    %         feats.(sprintf('Full_%s_aperiodic_intercept',r)) = intercept;
    %     else
    %         feats.(sprintf('Full_%s_aperiodic_slope',r)) = NaN;
    %         feats.(sprintf('Full_%s_aperiodic_intercept',r)) = NaN;
    %     end
    % end
    % 
    % % --------- wPLI (theta 4–7.5 Hz) FM↔Parietal and FM↔Central ---------------
    % %   • Uses FieldTrip's recommended multi-trial approach.
    % 
    % if size(win,2)/sr >= 20
    %     try
    %         % --- 4-s epochs with 2-s step (set step_s=4 for no overlap)
    %         win_s  = 4; step_s = 2;
    %         starts = 1:step_s*sr:(size(win,2) - win_s*sr + 1);
    % 
    %         ftdata = [];
    %         ftdata.label   = chanlabels(:);
    %         ftdata.fsample = sr;
    %         ftdata.trial   = cell(1,numel(starts));
    %         ftdata.time    = cell(1,numel(starts));
    %         for k = 1:numel(starts)
    %             idx = starts(k):(starts(k)+win_s*sr -1);
    %             ftdata.trial{k} = double(win(:,idx));
    %             ftdata.time{k}  = (0:win_s*sr-1)/sr;
    %         end
    % 
    %         % Fourier coefficients in theta with trials/tapers kept
    %         cfg = [];
    %         cfg.method      = 'mtmfft';
    %         cfg.output      = 'fourier';
    %         cfg.taper       = 'dpss';
    ...
    % 
    %         % Debiased wPLI with FT doing the freq average
    %         cfgc = [];
    %         cfgc.method      = 'wpli_debiased';
    %         cfgc.avgoverfreq = 'yes';
    %         conn = ft_connectivityanalysis(cfgc, freq);
    % 
    %         % Pull matrix + labels
    %         W = conn.wpli_debiasedspctrm;                 % <-- correct field
    %         if ndims(W) == 3, W = squeeze(mean(W,3,'omitnan')); end
    %         lab = conn.label;
    % 
    %         % ROI means (use labels from 'conn')
    %         fm_idx = find(ismember(lab, regs.FrontalMidline));
    %         p_idx  = find(ismember(lab, regs.Parietal));
    %         c_idx  = find(ismember(lab, regs.Central));
    % 
    %         feats.Full_wpli_fm_p_theta = mean(W(fm_idx,p_idx), 'all', 'omitnan');
    %         feats.Full_wpli_fm_c_theta = mean(W(fm_idx,c_idx), 'all', 'omitnan');
    % 
    %     catch ME
    %         warning('wPLI (FieldTrip) failed: %s', ME.message);
    %         feats.Full_wpli_fm_p_theta = NaN;
    %         feats.Full_wpli_fm_c_theta = NaN;
    %     end
    % else
    %     feats.Full_wpli_fm_p_theta = NaN;
    %     feats.Full_wpli_fm_c_theta = NaN;
