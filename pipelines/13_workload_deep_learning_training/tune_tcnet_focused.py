...
STUDY_NAME = 'eeg_tcnet_focused_tuning'
DB_PATH = f'sqlite:///{RESULTS_DIR}/optuna_focused.db'
...
CONVERGENCE_THRESHOLD = 0.62  # Lower threshold since baseline is 60%
...
def objective(trial):
    """Focused objective - only core architecture params."""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # ==== FOCUSED Hyperparameter search space ====
    # EEGNet architecture (most impactful)
    ...
    dropout_eeg = trial.suggest_float('dropout_eeg', 0.1, 0.4)
    ...
    dropout_tcn = trial.suggest_float('dropout_tcn', 0.2, 0.4)
    ...
    for fold, (train_idx, test_idx) in enumerate(create_group_splits(dataset, n_splits=N_FOLDS)):
        ...
        # Initialize model - FIXED: no attention
        model = EEGTCNet(
