...
run_single_fold <- function(train_df, test_df, target, pid) {
  ...
  dtrain <- xgb.DMatrix(as.matrix(train_df[, feats, drop = FALSE]), label = y_train)
  dtest  <- xgb.DMatrix(as.matrix(test_df[, feats, drop = FALSE]), label = y_test)
  
  # Inner CV for early stopping
  fold_idx <- make_pid_folds(train_df, k = 3)
  ...
  model <- xgb.train(params = params, data = dtrain, nrounds = nrounds, verbose = 3)
