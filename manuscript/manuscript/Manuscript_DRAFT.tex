...
Subjective ratings also showed moderate trait-like stability across the four blocks (stress ICC ≈ 0.47; mental-demand ICC ≈ 0.42), indicating that a meaningful portion of variance reflects stable individual differences in how participants appraise and report stress and workload. This helps explain why group-level manipulation effects can coexist with between-person heterogeneity that challenges subject-independent inference.

%replace unicode with latex for approx sign
...
At the physiological feature level, the pattern was asymmetric. Stress-related effects were expressed more consistently in autonomic channels, especially electrodermal activity, aligning with its established sensitivity to sympathetic arousal \cite{faircloughFundamentalsPhysiologicalComputing2009, masiStressWorkloadAssessment2023} and specifically to social-evaluative stress in TSST-based paradigms \cite{alonsoStressAssessmentBased2015, goodmanMetaanalyticalAssessmentEffects2017}. 

In contrast, MWL-related effects were weak in common univariate peripheral measures once physiological baselines were accounted for, suggesting that the autonomic signature of MWL is either subtle, context-dependent, or difficult to isolate in an active, socially salient VR setting. While heart rate variability and pupillometry have demonstrated sensitivity to cognitive load in controlled, sedentary settings \cite{hughesCardiacMeasuresCognitiveWorkload2019, vanderwelPupilDilationIndex2018, piquadoPupillometryMeasureCognitive2010, mathotPupillometryPsychologyPhysiology2017}, their discriminative capacity may be compromised in dynamic, multi-stressor environments; for pupillometry in particular, changes in scene luminance and arousal can dominate pupil dynamics in ways that are difficult to fully control in immersive settings \cite{panEmotionalArousalPupilLuminance2024, mathotPupillometryPsychologyPhysiology2017}.

MWL discriminability was most evident in neural features that emphasise relative spectral morphology rather than absolute band power. Specifically, frontal midline Theta/Beta and Alpha/Beta ratios both showed significant MWL main effects (both $p=.002$, $\eta^2=.07$), with High MWL associated with increased ratios relative to Low MWL. The Alpha/Beta increase may reflect greater frontal alpha synchronization under high internal-processing/top-down control demands \cite{benedekEEGAlphaSynchronization2011}, consistent with the mental-arithmetic manipulation here and potentially less generalisable to MWL tasks dominated by external processing.

%\cite{benedekEEGAlphaSynchronization2011} - Synchronization of EEG alpha activity has been referred to as being indicative of cortical idling, but according to more recent evidence it has also been associated with active internal processing and creative thinking. The main objective of this study was to investigate to what extent EEG alpha synchronization is related to internal processing demands and to specific cognitive process involved in creative thinking. To this end, EEG was measured during a convergent and a divergent thinking task (i.e., creativity-related task) which once were processed involving low and once involving high internal processing demands. High internal processing demands were established by masking the stimulus (after encoding) and thus preventing further bottom-up processing. Frontal alpha synchronization was observed during convergent and divergent thinking only under exclusive top-down control (high internal processing demands), but not when bottom-up processing was allowed (low internal processing demands). We conclude that frontal alpha synchronization is related to top-down control rather than to specific creativity-related cognitive processes. Frontal alpha synchronization, which has been observed in a variety of different creativity tasks, thus may not reflect a brain state that is specific for creative cognition but can probably be attributed to high internal processing demands which are typically involved in creative thinking.





This is consistent with broader workload EEG evidence linking MWL to systematic redistribution of spectral content \cite{chikhiEEGPowerSpectral2022} and to attentional control \cite{putman_eeg_2014}. A plausible mechanism is that ratios can reduce sensitivity to multiplicative broadband scaling (e.g., headset fit, impedance changes), though they can also be biased when artifacts differentially affect numerator and denominator; this is especially relevant under continuous speaking, where facial EMG can substantially contaminate higher-frequency EEG \cite{whithamScalpElectricalRecordingParalysis2007, deVosRemovalMuscleArtifactsSpokenLanguage2010}.
...
Despite the robust subjective MWL effect, our physiological features showed limited sensitivity to MWL once baseline differences were removed. Instead, the autonomic nervous system responses were dominated by the stress manipulation. Tonic EDA exhibited a clear elevation under high stress across both levels of task difficulty, consistent with its role as a robust indicator of sympathetic arousal \cite{faircloughFundamentalsPhysiologicalComputing2009, masiStressWorkloadAssessment2023, goodmanMetaanalyticalAssessmentEffects2017, manMultisystemicEvaluationBiological2023}. In contrast, we found a dissociation in the physiological signature of MWL. Raw univariate features (heart rate, HRV, pupil diameter, and canonical EEG band powers) did not significantly differ between High-MWL arithmetic and Low-MWL counting when controlling for baseline. In our immersive, social-evaluative context, these canonical raw signatures may have been attenuated or masked. For cardiovascular and pupillometric measures, one plausible contributor is context dependence and confounding (e.g., respiration/speech effects on HRV and luminance/arousal effects on pupil diameter), which can compress dynamic range and reduce specificity in active VR settings \cite{hughesCardiacMeasuresCognitiveWorkload2019, mathotPupillometryPsychologyPhysiology2017, panEmotionalArousalPupilLuminance2024}.

However, univariate analyses of EEG ratios (Theta/Beta and Alpha/Beta) revealed robust MWL effects ($p=.002$) that were absent in the raw power data. One plausible explanation is that some nuisance variance in absolute band power (e.g., multiplicative scaling, broadband shifts) is reduced when using relative spectral morphology, whereas higher-frequency bands can also be disproportionately affected by speech-related EMG contamination \cite{whithamScalpElectricalRecordingParalysis2007, deVosRemovalMuscleArtifactsSpokenLanguage2010}. Accordingly, we interpret ratio robustness as a practical advantage under these recording conditions rather than as a guarantee of artifact-free neural specificity.
...
The failure of raw EEG power to effectively index MWL in this study, which parallels findings in other active-task environments, contrasts sharply with the success of spectral ratios. Our sensitivity analysis provides a likely mechanism for this dissociation: broadband noise from speech production \cite{goncharova_emg_2003}, including facial EMG that can contaminate higher-frequency scalp EEG during overt speech \cite{whithamScalpElectricalRecordingParalysis2007, deVosRemovalMuscleArtifactsSpokenLanguage2010}. The significant effect of speech rate on the Alpha/Beta ratio confirms that verbal interaction introduces spectral interference that is not fully removed by standard cleaning. 

However, the key finding is that ratios retained their diagnostic utility despite this interference. While raw Frontal Theta power was statistically indistinguishable between conditions until the speech confound was explicitly modeled, the Theta/Beta and Alpha/Beta ratios successfully detected MWL even without such correction. This suggests that ratio-based metrics can behave as a partial normalization, reducing sensitivity to some shared variance in absolute power that arises from recording conditions. This robustness is particularly valuable for subject-independent systems operating in realistic environments where "clean" baselines are unavailable and sources of non-cognitive variance (stress, speech, movement) vary dynamically. By preserving the relative spectral morphology of the MWL response, ratios may offer a more stable target for classification than absolute power measures, consistent with their established role in indexing attentional control \cite{putman_eeg_2014}.
