...
Third, it incorporates machine-learning models alongside traditional inferential analyses, including group-level statistics (analyses of variance, correlations) and regression.
...
\item Pipeline – A multimodal acquisition and preprocessing framework (EEG, EDA, HR/HRV, pupil) designed for subject-independent modelling.
...
After automated signal-quality checks and dataset validation, 44 participants were retained for analysis (mean age = 31.0 years, standard deviation (SD) = 6.2; 20 female). 
...
For EEG, participants were excluded if, after preprocessing, less than $75\%$ of data samples were retained, more than $15\%$ of channels were interpolated, or over $35\%$ of independent components were removed. EEG waveform plots were automatically generated and visually inspected by one researcher after each major cleaning step (channel rejection and ICA component removal) to confirm satisfactory data quality.
...
\begin{table}[!t]
\centering
\caption{Post hoc contrast and cell means (with standard errors (SE)) for tonic EDA (mean, $\mu$S).}
\label{tab:eda_posthoc}
\begin{tabular}{lcccc}
\toprule
\multicolumn{5}{c}{\textbf{Post hoc contrast (Stress main effect)}} \\
\midrule
Contrast & Estimate & SE & $t$(129) & $p$ \\
\midrule
Low -- High & -12.16 & 5.74 & -2.12 & 0.036 \\
\midrule
...
\midrule
Low  & Low  & 0.711 & 0.123 & \\
Low  & High & 1.073 & 0.186 & \\
High & Low  & 1.229 & 0.168 & \\
High & High & 1.238 & 0.215 & \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent\textit{Note.} Metrics are averaged across 44 LOSO folds (one per EEG-valid participant).
...
Despite the strong subjective MWL effect, physiological sensitivity to workload was minimal after baseline correction, whereas stress effects were modest. Tonic EDA showed a small-to-moderate elevation under high stress (F=3.80, p=.053), and heart rate showed a trend-level stress effect. This finding is consistent with the role of EDA as an indicator of sympathetic arousal: the social-evaluative threat in the high-stress VR scenarios evoked
sustained sympathetic activation, which was reflected in higher skin conductance level. The tonic EDA elevation was consistent across workload levels but modest in magnitude. Notably, this stress effect in EDA was small-to-moderate in magnitude, indicating that while reliable, it did not involve a dramatic physiological surge for all participants. Other autonomic indices such as heart rate and HRV (RMSSD) did not show significant overall differences between high- and low-stress conditions, although heart rate (median) showed a trend-level stress effect.
...
By examining within-condition correlations, we probed whether physiology–state relationships hold when the other factor is fixed. The stratified repeated-measures results (modality-family FDR) mirrored the main correlation findings: under low workload, stress was positively associated with heart rate (median), tonic EDA, and pupil diameter; under high workload, tonic EDA showed a trend-level positive association with stress. No MWL correlations or EEG correlations survived correction.
...
From a modelling and validation perspective, while we used a nested leave-one-subject-out framework for tuning and evaluation, performance estimates are still high-variance in small samples and would benefit from uncertainty quantification (e.g., bootstrap confidence intervals) and replication across sessions and contexts to better characterise generalisability. Despite these constraints, this study offers a valuable initial demonstration of multimodal physiological detection of stress and \(\mathrm{MWL}\) in immersive contexts, and the insights we gained—regarding feature behaviour, modality synergy and pilot model feasibility—remain valid within the defined scope. Addressing the outlined limitations in follow-up work will enhance the reliability, external validity and practical applicability of adaptive affective‐computing systems in real-world settings.
