function [EEG, qc] = clean_eeg(raw_set_path, output_folder, participant_num, vis_folder, qc_folder, config)
    ...
    % Validate sample mask (flagging, not removal) and persist for downstream use
    ...
    perc_asr_repaired = 100 * mean(~asr_mask);
    log_message(logfile, sprintf('ASR flagging: %.2f%% samples repaired/flagged (no window deletions).', perc_asr_repaired));

    % Save mask for later ML/QC use
    asr_mask_dir = fullfile(qc_folder, 'asr_masks');
    if ~exist(asr_mask_dir, 'dir')
        mkdir(asr_mask_dir);
    end
    save(fullfile(asr_mask_dir, sprintf('P%02d_clean_sample_mask.mat', participant_num)), 'asr_mask');
