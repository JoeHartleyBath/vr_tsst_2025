...
Requirements:
    ...
    - Baseline data: data/subjective_baseline_measures.xlsx
...
def main():
    ...
    # Load counterbalance data
    print("Loading counterbalance data...")
    
    # Updated path - counterbalance file is in data/ root
    counterbalance_path = "data/experimental_counterbalance.xlsx"
    ...
    # Check for optional baseline file
    baseline_path = "data/subjective_baseline_measures.xlsx"
    if os.path.exists(baseline_path):
        baseline_data = pd.read_excel(baseline_path)
        print(f"  ✓ Loaded baseline data: {len(baseline_data)} participants")
        use_baseline = True
    else:
        print(f"  ⚠ Baseline data not found - skipping baseline adjustment")
    ...
    for participant_number in participants:
        try:
            # Construct input file path (with zero-padding for P01-P09)
            file_path = f"data/RAW/subjective/PQs_{participant_number:02d}_compiled.csv"
            ...
            # Baseline adjustment (only if baseline data available)
            if use_baseline:
            ...
            for idx, condition_short in enumerate(participant_conditions):
                ...
                # Add affect ratings (baseline-adjusted if available, otherwise raw)
                for metric in subtract_columns:
                    if use_baseline:
                        ...
                        # Use raw values if no baseline
                        if metric in df.columns:
                            row[metric] = df.loc[idx, metric]
                        elif metric == 'Pleasure' and 'Valence' in df.columns:
                            row[metric] = df.loc[idx, 'Valence']
                        else:
                            row[metric] = None
